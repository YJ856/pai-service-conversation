...
import {
  ChildProfileSummary,
  UserApiPort,
} from 'src/application/port/out/user-api.port';
...
export class ProfileDirectoryHttpAdapter implements UserApiPort {
  ...
  async getFamilyProfileWithScopeChildren(): Promise<{
    ...
    try {
      ...
      this.logger.warn(
        `getFamilyProfileWithScopeChildren failed: ${String(error)}`,
      );
      return { children: [] };
    }
  }
...
function parseBigIntOrNull(input: unknown): bigint | null {
  ...
  if (typeof input !== 'string' && typeof input !== 'number') return null; // string, number만 허용
