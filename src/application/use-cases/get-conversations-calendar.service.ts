...
import type { ChildProfileSummary, UserApiPort } from "../port/out/user-api.port";
...
export class GetConversationsCalendarService implements GetConversationsCalendarUseCase {
    ...
    async execute(command: GetConversationsCalendarCommand): Promise<GetConversationsCalendarResult> {
        const { year, month, childProfileId } = command;
        ...
        const { children } = await this.profiles.getFamilyProfileWithScopeChildren();
        const childList = (children ?? []) as ChildProfileSummary[];
        ...
        // 아이 정보 빠르게 찾는 맵 만들기
        const childById: Record<number, ChildProfileSummary> = 
            Object.fromEntries(childList.map(child => [child.profileId, child]));
        ...
        // 날짜별 합계 만들기
        const countsByDate = new Map<string, { count: number; children: Map<number, number> }>();
        let totalCount = 0;
        ...
        for (const row of rows) {
            ...
            bucket.children.set(row.childProfileId, (bucket.children.get(row.childProfileId) ?? 0) + row.count);
        }
        ...
        const days = Array.from(countsByDate.entries())
            ...
            .map(([date, { count, children }]) => ({
                ...
                children: Array.from(children.entries())
                    .sort(([childId1, count1], [childId2, count2]) => (count2 - count1) || (childId1 - childId2))
                    .map(([childProfileId, cnt]) => ({
                        childProfileId: childProfileId,
                        childName: childById[childProfileId]?.name ?? "",
                        childAvatarMediaId: (childById[childProfileId]?.avatarMediaId ?? null) as bigint | null,
                        count: cnt,
                    })),
            }));
